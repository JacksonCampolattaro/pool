...
def generate_input_data(m: int = 8192, n: int = 4096, k: int = 24, c: int = 128, device: str = 'cpu'):
    features = torch.rand([m, c]).to(device=device)
...
def test_naive_pool(benchmark, device: str, config: tuple):
    m, n, k, c = config
...
@parameterizeByDevice
@pytest.mark.parametrize('config', TEST_CONFIGS_M_N_K_C)
def test_pyg_pool(benchmark, device, config: tuple):
...
def to_sparse_edges(features, neighbors):
    return SparseTensor(
        row=torch.arange(neighbors.size(0), device=neighbors.device, dtype=neighbors.dtype).unsqueeze(-1).expand(
            [-1, neighbors.size(1)]).flatten(),
...
@parameterizeByDevice
@pytest.mark.parametrize('config', TEST_CONFIGS_M_N_K_C)
def test_sparse_pool(benchmark, device, config: tuple):
    features, neighbors, _ = generate_input_data(*config, device=device)
...
@parameterizeByDevice
@pytest.mark.parametrize('config', TEST_CONFIGS_M_N_K_C)
def test_tosparse_pool(benchmark, device, config: tuple):
    features, neighbors, _ = generate_input_data(*config, device=device)
    conv = SimpleConv(aggr='max')
